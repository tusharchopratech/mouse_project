...
class Report extends Component {
  ...
  componentDidMount = () => {
    ...
    if (this.props.report != null) {
      this.setState({
        leftTruePositives:  Math.round(this.props.report.left_click.true_positives * 100),
        leftFalsePositives: Math.round(this.props.report.left_click.false_positives* 100),
        leftAverageLead: Math.round(this.props.report.left_click.average_lead* 1000),
        rightTruePositives: Math.round(this.props.report.right_click.true_positives* 100),
        rightFalsePositives: Math.round(this.props.report.right_click.false_positives* 100),
        rightAverageLead: Math.round(this.props.report.right_click.average_lead* 1000)
      });
    }
  };
    ...
    return (
      <div>
        <div style={{display: "flex", flexDirection: "column", width: "100%", alignItems: "center"}}>
          ...
          <div>Average Lead : {this.state.leftAverageLead} ms.</div> 
          <br></br>
          ...
          <div>Average Lead : {this.state.rightAverageLead} ms.</div> 
        </div>
        <br></br>
